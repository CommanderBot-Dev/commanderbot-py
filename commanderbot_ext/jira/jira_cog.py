...
class JiraCog(Cog):
    ...
    async def cmd_jira(self, ctx: discord.Message, bug_id: str):
        ...
        try:
            report_data = requests.get(f"https://bugs.mojang.com/rest/api/latest/issue/{bug_id}").json()["fields"]
        # Bug report doesn't exist
        except KeyError:
            await ctx.send(f"**{bug_id.upper()}** is not accessible. This may be due to it being private or it may not exist.")
            return
        ...
        jira_embed = discord.Embed(title=title, url=f"https://bugs.mojang.com/browse/{bug_id}", color=0x00aced)
        ...
        jira_embed.set_footer(text=str(ctx.message.author), icon_url=str(ctx.message.author.avatar_url))
